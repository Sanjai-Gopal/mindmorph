...
import type { AchievementItem } from "@/components/features/Gamification/AchievementGrid";
...
const AchievementGrid = dynamic(
  () => import("@/components/features/Gamification/AchievementGrid").then((mod) => mod.AchievementGrid),
  { ssr: false }
);
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const longestSessionMinutes = sessions.reduce(
        (max, s) => Math.max(max, Math.round(Number(s.duration_seconds || 0) / 60)),
        0
      );
      ...
      setAchievements(
        ACHIEVEMENTS.map((achievement) => ({
          ...
          progress: unlocked.has(achievement.id)
            ? 100
            : gamificationEngine.getAchievementProgress(snapshot, achievement.id),
          unlockedAt: unlocked.has(achievement.id) ? new Date().toISOString() : null,
          recent: newAchievements.includes(achievement.id)
        }))
      );
      if (newAchievements[0]) {
        const unlockedAchievement = ACHIEVEMENTS.find((a) => a.id === newAchievements[0]);
        setCelebration({ open: true, title: unlockedAchievement?.title ?? "Achievement" });
      }
      setLoading(false);
    };
    void load();
